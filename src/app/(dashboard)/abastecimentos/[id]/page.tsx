...
import { getRefuelById, updateRefuel } from '@/services/refuels';
...
// ============================================================
// EDIT ABASTECIMENTO PAGE
// ============================================================
export default function EditAbastecimentoPage() {
    ...
    Promise.all([
      ...
      .then(([vList, sList, refuel]) => {
        ...
        if (refuel) {
          ...
          reset({
            date: refuel.date,
            fuelType: refuel.fuelType as RefuelFormData['fuelType'],
            previousOdometer: refuel.previousOdometer,
            currentOdometer: refuel.currentOdometer,
            liters: refuel.liters,
            unitPrice: refuel.unitPrice,
            vehicleId: refuel.vehicleId,
            vehiclePlate: refuel.vehiclePlate,
            vehiclePrefix: refuel.vehiclePrefix,
            vehicleModel: refuel.vehicleModel,
            vehicleBranch: refuel.vehicleBranch,
            vehicleBranchId: refuel.vehicleBranchId ?? '',
            stationId: refuel.stationId,
            stationName: refuel.stationName,
            city: refuel.city,
            state: refuel.state ?? '',
            paymentMethod: refuel.paymentMethod as any ?? '',
            invoiceNumber: refuel.invoiceNumber ?? '',
            observations: refuel.observations ?? '',
          });
          
          setSelectedVehicle(vList.find(v => v.id === refuel.vehicleId) || null);
          setSelectedStation(sList.find(s => s.id === refuel.stationId) || null);
        } else {
  ...
  const doSubmit = async (data: RefuelFormData) => {
    ...
    try {
      ...
      await createAuditLog(
        ...
        'UPDATE', 'refuel', id,
        `${profile.name} editou o abastecimento: ${data.vehiclePlate}`,
